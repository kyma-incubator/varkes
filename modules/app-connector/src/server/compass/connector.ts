...
async function getConfiguration(token: string, url: string, insecure: boolean): Promise<any> {
  ...
  var result = await common.createConnectorClient(token, url, insecure).query({
    query: configurationQuery
  }).catch(err => {
    if (err.networkError) {
      LOGGER.debug(`Detailed network error: ${JSON.stringify(err.networkError, null, 2)}`)
      throw new Error(`Getting configuration with URL '${url}' failed with status '${err.networkError.statusCode}' and message '${err.message}'`)
    } else {
...
async function signCertificateSigningRequest(url: string, insecure: boolean, configuration: any) {
  const SignCertificateSigningRequestMutation = gql`mutation($csr: String!) {
  ...
  var result = await common.createConnectorClient(configuration.token.token, url, insecure).mutate({
    ...
    if (err.networkError) {
      LOGGER.debug(`Detailed network error: ${JSON.stringify(err.networkError, null, 2)}`)
      throw new Error(`Sending CSR with URL '${url}' failed with status '${err.networkError.statusCode}' and message '${err.message}'`)
    } else {
      LOGGER.debug(`Detailed graphQL error: ${JSON.stringify(err.graphQLErrors, null, 2)}`)
...
export async function eventsUrl(): Promise<string> {
  ...
  LOGGER.debug(`Received eventsURL Result = ${result.data.application ? JSON.stringify(result.data.application.eventingConfiguration, null, 2) : " application not found"}`)
...
async function queryAppID(connectionInfo: connection.Info, certificate: Buffer): Promise<string> {
  ...
  let result = await common.createDirectorClientWithInfo(connectionInfo, certificate).query({
    query: query
  }).catch(err => {
    if (err.networkError) {
      LOGGER.debug(`Detailed network error: ${JSON.stringify(err.networkError, null, 2)}`)
      throw new Error(`viewer query failed with status '${err.networkError.statusCode}' and message '${err.message}'`)
    } else {
      LOGGER.debug(`Detailed graphQL error: ${JSON.stringify(err.graphQLErrors, null, 2)}`)
...
export async function connect(token: string, insecure: boolean = false): Promise<any> {
  let connectorUrl
  let connectorToken
